...
import { 
  FileText, User, MapPin, Clock, CheckCircle, Printer, Send, 
  ArrowLeft, Loader2, Shield, HeartPulse, AlertTriangle, Stethoscope, FileDown, RotateCcw 
} from "lucide-react";
import { TopBar } from "../../components/layout/TopBar";
import { FormInput, FormTextarea, FormSelect } from "../../components/ui/FormInput";
import { generateHandoverPDF, PCRReportData } from "../../lib/pdfExport";
...
export default function TurnoverPage() {
  ...
  const [circulationStatus, setCirculationStatus] = useState("Strong/Regular");
  const [skinCondition, setSkinCondition] = useState("Normal");
  const [pupilResponse, setPupilResponse] = useState("PEARL");
  ...
  const [ivAccess, setIvAccess] = useState("None");
  const [oxygenTherapy, setOxygenTherapy] = useState("None");
  const [immobilization, setImmobilization] = useState("None");
        ...
        <header data-animate className="mb-8 flex flex-col md:flex-row md:items-center justify-between gap-4">
          <div>
            <div className="flex items-center gap-2">
              <Link href="/responder/qr-scan" className="text-[#8b949e] hover:text-white transition-colors mr-2">
