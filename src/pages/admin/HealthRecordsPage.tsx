...
/** Shape of a decrypted vitals payload from core.residents.encrypted_payload */
...
/** Shape of a row in the health records table (used in list view) */
...
/** Shape of a row from health.records (Supabase live query) */
...
/** Shape of a row from core.residents (Supabase live query) */
...
/** Shape of a row from security.users (Supabase live query) */
interface SecurityUserRow {
  user_id: string;
  full_name: string;
}

/** Shape of a cached resident row from localStorage */
interface LocalResidentEntry {
  id?: string;
  name?: string;
  age?: number;
  dob?: string;
...
/** Shape of a cached handover from localStorage */
interface LocalHandoverEntry {
  id?: string;
  patient_name?: string;
...
// ─── Default mock records for offline/placeholder mode ────────────────────────
...
const riskMap: Record<string, "red" | "orange" | "green"> = {
  High:   "red",
  Medium: "orange",
  Low:    "green",
};
...
export default function HealthRecordsPage() {
  ...
  const [stats, setStats] = useState({ highRisk: 234, chronic: 891, consults: 47 });

  // Modal state — fully typed, no any
  const [selectedRecord, setSelectedRecord] = useState<HealthRecord | null>(null);
  const [decryptedMedical, setDecryptedMedical] = useState<DecryptedVitals | null>(null);
  ...
  const loadHealthRecords = async () => {
    if (isPlaceholderUrl) {
      ...
      const parsed = JSON.parse(cached) as LocalResidentEntry[];

      const mapped = await Promise.all(
        parsed.map(async (r, idx): Promise<HealthRecord> => {
          let condition = "None on record";
          let bpVal = "120/80";
          let bmiVal = "22.5";
          let ageVal = typeof r.age === "number" ? r.age : 30;
          ...
          if (r.encryptedPayload) {
            try {
              const decrypted = (await decryptPayload(r.encryptedPayload, "barangay45key")) as DecryptedVitals;
              if (decrypted) {
                condition = decrypted.chronic || decrypted.sample?.s || "None on record";
                bpVal = decrypted.vitals?.bp || "120/80";
                bmiVal = calcBmi(decrypted.vitals?.height, decrypted.vitals?.weight);
              }
            } catch (e) {
              console.warn("Failed to decrypt profile for health records list", e);
            }
          }
          ...
          return {
            ...
            risk: (r.vulnerability as RiskLevel) || "Low",
            encryptedPayload: r.encryptedPayload,
          };
        })
      );

      const localHandovers = JSON.parse(
        localStorage.getItem("respondaCare_handovers") || "[]"
      ) as LocalHandoverEntry[];

      const handoverRecords: HealthRecord[] = localHandovers.map((h, idx) => ({
        id: h.id || `RC-H-${idx}`,
        ...
        risk: (h.severity_score ?? 0) >= 8 ? "High" : (h.severity_score ?? 0) >= 4 ? "Medium" : "Low",
        encryptedPayload: "",
      }));
      ...
      setStats({
        highRisk: combined.filter((r) => r.risk === "High").length,
        chronic:  combined.filter((r) => r.condition !== "None on record" && r.condition !== "None").length,
    ...
    try {
      const { data: recData, error: recErr } = await supabase
        .schema("health")
        .from("records")
        .select("record_id, resident_id, encrypted_vitals, record_type, created_at")
      ...
      const { data: resData } = await supabase
        .schema("core")
        .from("residents")
        .select("resident_id, user_id, mobility_status, date_of_birth");
      ...
      const resMap = new Map<string, ResidentRow>(
        (resData as ResidentRow[] | null ?? []).map((r) => [r.resident_id, r])
      );
      const userMap = new Map<string, SecurityUserRow>(
        (usersData as SecurityUserRow[] | null ?? []).map((u) => [u.user_id, u])
      );

      const mapped = await Promise.all(
        (recData as HealthRecordRow[] | null ?? []).map(async (rec): Promise<HealthRecord> => {
          const resident = resMap.get(rec.resident_id);
          const user = resident ? userMap.get(resident.user_id) : undefined;

          const birthYear = resident?.date_of_birth
          ...
          try {
            const decrypted = (await decryptPayload(rec.encrypted_vitals, "barangay45key")) as DecryptedVitals;
            if (decrypted) {
              condition = decrypted.sample?.s || decrypted.chronic || "None on record";
              bpVal = decrypted.vitals?.bp || "120/80";
              bmiVal = calcBmi(decrypted.vitals?.height, decrypted.vitals?.weight);
            }
          } catch {
            // Ignore decryption failure for list mapping — show redacted defaults
          }

          const mobilityStatus = resident?.mobility_status ?? "Low";
          const risk: RiskLevel =
            mobilityStatus === "High" ? "High" :
            mobilityStatus === "Medium" ? "Medium" : "Low";
      ...
      setStats({
        highRisk: mapped.filter((r) => r.risk === "High").length || 234,
        chronic:  mapped.filter((r) => r.condition !== "None on record").length || 891,
        consults: (recData as HealthRecordRow[]).filter(
          (rec) => new Date(rec.created_at) >= sevenDaysAgo
        ).length || 47,
      });
    } catch (err: unknown) {
      console.error("Error fetching live health records:", err instanceof Error ? err.message : err);
    }
  };
  ...
  const handleOpenModal = async (rec: HealthRecord) => {
    ...
    if (rec.encryptedPayload) {
      setDecrypting(true);
      try {
        const decrypted = (await decryptPayload(rec.encryptedPayload, "barangay45key")) as DecryptedVitals;
        setDecryptedMedical(decrypted);
      } catch (err: unknown) {
        console.error("Error decrypting medical record detail:", err instanceof Error ? err.message : err);
      } finally {
        setDecrypting(false);
      }
    }
            ...
            <div className="relative w-full max-w-xl bg-[#1a1d23] border border-gray-800 rounded-2xl shadow-2xl overflow-hidden max-h-[90vh] flex flex-col text-sm">
              <header className="px-6 py-4 bg-[#1e222a] border-b border-gray-800 flex justify-between items-center">
                <div>
                  ...
                  <p className="text-xs text-[#8b949e] mt-1 font-mono">Resident: {selectedRecord.name}</p>
                </div>
                <button
                  onClick={() => { setSelectedRecord(null); setDecryptedMedical(null); }}
                ...
                <div>
                  ...
                  {decrypting ? (
                    <div className="p-8 bg-[#0d1117] rounded-xl border border-white/[0.02] flex flex-col items-center justify-center gap-2">
                      ...
                      <p className="text-xs text-gray-400 font-mono">Decrypting payload…</p>
                    </div>
                  ) : decryptedMedical ? (
                    <div className="bg-[#0d1117] p-5 rounded-xl border border-white/[0.02] space-y-4">
                      ...
                      {decryptedMedical.notes && (
                        <div className="border-t border-white/[0.04] pt-4">
                          <p className="text-xs text-gray-500 font-bold uppercase">BHW Notes</p>
                    ...
                    <div className="bg-[#0d1117] p-5 rounded-xl border border-white/[0.02] text-[#8b949e] text-xs text-center">
                      No encrypted checkup data found. Showing default mock assessments.
                    </div>
                  )}
                </div>
              </div>
